...
detections_by_Species_Year <- df.alltags.912.clean |>
  ...
  mutate(
    avgHitsPerTag = paste0(round(avgHitsPerTag, 2), " ± ", round(avgHitsPerTag_SD, 2)),  # Combine mean and SD
    avgDaysDetected = paste0(round(avgDaysDetected, 2), " ± ", round(avgDaysDetected_SD, 2)),  # Combine mean and SD
    meanNRecv = paste0(round(meanNRecv, 2), " ± ", round(nRecv_sd, 2))
  ) |>
...
detections_by_Species_Year <- detections_by_Species_Year |>
  ...
  select(tagDepYear, speciesEN, nTagsDeployed, nTagsDetected, percentTagsDetected, nHitsAllTags, avgHitsPerTag, avgDaysDetected, meanNRecv) |>
...
detections_by_tagModel <- detections_by_tagModel |>
  ...
  select(tagModel, nTagsDeployed, nTagsDetected, nHits, avgHitsPerTag, avgDaysDetected, nRecv) |>
  mutate(percentTagsDetected = (nTagsDetected/nTagsDeployed)*100) |>
  select(tagModel, nTagsDeployed, nTagsDetected, percentTagsDetected, nHits, avgHitsPerTag, avgDaysDetected, nRecv) |>
  mutate(across(where(is.numeric), ~ round(., 1)))
...
```{r CCLO individual maps, echo = FALSE, message = FALSE, warning = FALSE, fig.show = "hold", results = 'hide', fig.width=10, fig.height=5, fig.align='left', fig.cap="Figure 10. Apparent migration paths for two Chestnut-collared Longspurs that were detected in probable overwintering locations."}
...
```{r HOLA individual maps, echo = FALSE, message = FALSE, warning = FALSE, fig.show = "hold", results = 'hide', fig.width=8, fig.height=4, fig.align='left', fig.cap="Figure 11. Apparent migration paths for two Horned Larks."}
...
```{r SPPI individual maps, echo = FALSE, message = FALSE, warning = FALSE, fig.show = "hold", results = 'hide', fig.width=8, fig.height=4, fig.align='left', fig.cap="Figure 12. Apparent migration paths for four Sprague's Pipits that were detected in probable overwintering locations. Solid lines represent fall migratory movements and dashed lines represent spring migratory movements."}
